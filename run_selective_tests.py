...
standard_models = list(set(working_models) & set(["sonar-pro", "sonar"]))
if standard_models:
    patterns = [f"test_standard_models[{model}-" for model in standard_models]
    expressions.extend(patterns)

# Other models test
other_models = list(set(working_models) & set(["sonar-deep-research", "sonar-reasoning-pro"]))
if other_models:
...
if "sonar-pro" in working_models:
    expressions.extend([
        ...
        "test_citations_handling",
    ])

# Check if OpenRouter key is set and add that test if so
if os.environ.get("LLM_OPENROUTER_KEY") and "sonar" in working_models:
